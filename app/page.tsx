  ...
  return (
    <div>
      ...
      <section style={sx('background:#1279BF;color:#fff;position:relative;overflow:hidden')}>
        ...
        <div style={sx('max-width:1220px;margin:0 auto;padding:clamp(40px,7vw,84px) 20px clamp(40px,6vw,72px);display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr));gap:40px;align-items:center;position:relative')}>
          <div>
            ...
            <h1 style={sx('font-size:clamp(34px,5.2vw,60px);font-weight:800;letter-spacing:-.01em;margin-bottom:18px')}>Air conditioning & electrical.<br />One licensed team, Brisbane to Gold Coast.</h1>
            ...
            <div style={sx('display:flex;flex-wrap:wrap;gap:12px;margin-bottom:28px')}>
              <QuoteButton className="hv-redd" style={sx('background:#E32027;color:#fff;border:0;border-radius:12px;padding:16px 26px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.18)')}>Get a Free Quote</QuoteButton>
              <a href="tel:1300010393" className="hv-mint" style={sx('background:#fff;color:#0E2A47;border-radius:12px;padding:16px 24px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px;display:flex;align-items:center;gap:8px')}>☏ 1300 010 393</a>
            </div>
            <ul style={sx('list-style:none;padding:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 18px;font-size:15px;color:#E3F0FA;max-width:520px')}>
      ...
      <section style={sx('background:#fff;border-bottom:1px solid #E1E8F0')}>
        <div style={sx('max-width:1220px;margin:0 auto;padding:18px 20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px;text-align:center')}>
          {TRUST_STATS.map((s) => (
            <div key={s.l} style={sx('padding:8px 4px')}>
              <div style={sx('font-family:Barlow,sans-serif;font-weight:800;font-size:26px;color:#1279BF;line-height:1')}>{s.v}</div>
      ...
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px 0')}>
        <div style={sx('text-align:center;max-width:680px;margin:0 auto 36px')}>
          <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>What we can offer you</div>
          <h2 style={sx('font-size:clamp(28px,3.4vw,40px);font-weight:700')}>Fast, reliable electrical and air conditioning services</h2>
        </div>
        <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:20px')}>
          {PILLARS.map((p) => (
      ...
      {/* WHAT WE DO GRID */}
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px 0')}>
        <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:36px;align-items:start')}>
          <div>
            <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>One team for everything</div>
          ...
          <div style={sx('display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px')}>
            {WHAT_WE_DO.map((w) => (
              <Link key={w.n} href={w.href} className="hv-bord" style={sx('background:#fff;border:1px solid #E1E8F0;border-radius:14px;padding:16px;display:flex;flex-direction:column;gap:8px;color:#14263A;min-height:104px')}>
                <span style={sx('width:34px;height:34px;border-radius:9px;background:#EAF3FA;color:#1279BF;display:flex;align-items:center;justify-content:center;font-family:Barlow,sans-serif;font-weight:800;font-size:15px')}>{w.n}</span>
      ...
      <section style={sx('background:#0E2A47;color:#fff;margin-top:clamp(48px,6vw,80px)')}>
        <div style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:40px;align-items:center')}>
          <div>
            <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#9BE04A;margin-bottom:10px')}>Why choose Links Air?</div>
          ...
          <div style={sx('display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px')}>
            {USPS.map((u) => (
              <div key={u.t} style={sx('background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:18px 16px')}>
                <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:18px;margin-bottom:4px')}>{u.t}</div>
      ...
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px 0')}>
        <div style={sx('display:flex;flex-wrap:wrap;justify-content:space-between;align-items:flex-end;gap:16px;margin-bottom:28px')}>
          <div>
            <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>Testimonials</div>
        ...
        <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,280px),1fr));gap:16px')}>
          {TESTIMONIALS.map((t) => (
        ...
        <section style={sx('max-width:1220px;margin:clamp(48px,6vw,80px) auto 0;padding:0 20px')}>
          <div style={sx('background:#E32027;color:#fff;border-radius:20px;padding:clamp(28px,4vw,44px);display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:20px')}>
            <div style={sx('display:flex;align-items:center;gap:22px;flex-wrap:wrap')}>
              <div style={sx('font-family:Barlow,sans-serif;font-weight:800;font-size:clamp(48px,6vw,72px);line-height:1;letter-spacing:-.02em')}>20% OFF</div>
              <div>
                <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:clamp(20px,2.4vw,26px)')}>Electrical or AC maintenance & repair</div>
            ...
            <div style={sx('display:flex;gap:10px;flex-wrap:wrap')}>
              <a href="tel:0447440050" style={sx('background:#fff;color:#E32027;border-radius:12px;padding:14px 22px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px')}>☏ 0447 440 050</a>
              <QuoteButton className="hv-navy" style={sx('background:#0E2A47;color:#fff;border:0;border-radius:12px;padding:14px 22px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px;cursor:pointer')}>Schedule an appointment</QuoteButton>
            </div>
          </div>
        </section>
      ) : null}
      ...
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px 0')}>
        <div style={sx('text-align:center;max-width:680px;margin:0 auto 28px')}>
          <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>Work gallery</div>
      ...
      {/* AREAS + FAQ */}
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,80px) 20px 0;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr));gap:40px')}>
        <div>
          <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>Areas we service</div>
        ...
        <div>
          <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#E32027;margin-bottom:10px')}>FAQs</div>
          ...
          <div style={sx('display:flex;flex-direction:column;gap:8px')}>
            {HOME_FAQ.map((f) => (
              <details key={f.q} style={sx('background:#fff;border:1px solid #E1E8F0;border-radius:12px;padding:0 18px')}>
                <summary style={sx('padding:15px 0;font-weight:700;font-size:16px;display:flex;justify-content:space-between;gap:12px;align-items:center')}>{f.q}<span style={sx('color:#1279BF;font-size:18px')}>+</span></summary>
      ...
      <section style={sx('max-width:1220px;margin:0 auto;padding:clamp(48px,6vw,72px) 20px 0')}>
        <p style={sx('text-align:center;font-family:Barlow,sans-serif;font-weight:700;font-size:13px;letter-spacing:.14em;text-transform:uppercase;color:#7A8A9C;margin-bottom:18px')}>We supply, install and service top brands</p>
        ...
        <div style={sx('background:#1279BF;color:#fff;border-radius:22px;padding:clamp(32px,5vw,56px);text-align:center;position:relative;overflow:hidden')}>
          ...
          <div style={sx('position:relative')}>
            ...
            <div style={sx('display:flex;flex-wrap:wrap;gap:12px;justify-content:center')}>
              <a href="tel:1300010393" style={sx('background:#fff;color:#0E2A47;border-radius:12px;padding:16px 26px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px')}>Call Links Air Electrical</a>
              <QuoteButton className="hv-redd" style={sx('background:#E32027;color:#fff;border:0;border-radius:12px;padding:16px 26px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px;cursor:pointer')}>Request a Free Quote</QuoteButton>
            </div>
          </div>
        </div>
      </section>
