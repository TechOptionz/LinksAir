...
export default function ContactPage() {
  return (
    <div>
      ...
      <div style={sx('max-width:1220px;margin:0 auto;padding:clamp(32px,5vw,64px) 20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr));gap:36px;align-items:start')}>
            ...
            const inner = (
              <>
                <div>
                  ...
                  <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:22px;color:#14263A')}>{card.v}</div>
                </div>
                <span style={sx('color:#1279BF;font-size:20px')}>→</span>
              </>
            );
            const style = sx('background:#fff;border:1px solid #E1E8F0;border-radius:16px;padding:20px 22px;display:flex;justify-content:space-between;align-items:center;gap:16px;color:#14263A');
