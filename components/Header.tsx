    ...
    <header style={sx('background:#fff;position:sticky;top:0;z-index:50;box-shadow:0 1px 0 #E1E8F0')} onMouseLeave={() => setOpenMenu(null)}>
      <div style={sx('max-width:1220px;margin:0 auto;padding:0 20px;display:flex;align-items:center;justify-content:space-between;gap:16px;height:72px')}>
        ...
        <nav aria-label="Primary" className="lae-desktop" style={sx('display:flex;align-items:center;gap:2px;height:100%')}>
          {NAV.map((item, i) => (
            <div key={item.label} style={sx('position:relative;height:72px;display:flex;align-items:center')} onMouseEnter={() => setOpenMenu(item.groups ? i : null)}>
              <Link href={item.href} className="hv-bb" style={sx('font-family:Barlow,sans-serif;font-weight:600;font-size:15px;letter-spacing:.01em;color:#14263A;padding:10px 10px;border-radius:8px;display:flex;align-items:center;gap:4px;white-space:nowrap')}>
                {NAV_SHORT[item.label] || item.label}
                {item.groups ? <span style={sx('font-size:9px;opacity:.55')}>▼</span> : null}
              </Link>
              {openMenu === i && item.groups ? (
                <div style={sx('position:absolute;top:66px;left:0;background:#fff;border:1px solid #E1E8F0;border-radius:14px;box-shadow:0 18px 40px rgba(14,42,71,.14);padding:18px;display:flex;gap:26px;animation:lae-fade .18s ease;z-index:60;min-width:260px')}>
                  {item.groups.map((g) => (
                    <div key={g.label} style={sx('min-width:220px')}>
                      {g.href ? (
                        <Link href={g.href} className="hv-bd" style={sx('display:block;font-family:Barlow,sans-serif;font-weight:700;font-size:14px;text-transform:uppercase;letter-spacing:.06em;color:#1279BF;padding:4px 8px 8px;border-bottom:2px solid #E8F1F8;margin-bottom:6px')}>{g.label}</Link>
                      ) : (
                        <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:14px;text-transform:uppercase;letter-spacing:.06em;color:#5B6E82;padding:4px 8px 8px;border-bottom:2px solid #E8F1F8;margin-bottom:6px')}>{g.label}</div>
                      )}
                      {g.children.map((ch) => (
        ...
        <div className="lae-desktop" style={sx('display:flex;align-items:center;gap:8px;flex-shrink:0')}>
          <a href="tel:1300010393" aria-label="Call 1300 010 393" className="hv-bb" style={sx('display:flex;align-items:center;gap:8px;font-family:Barlow,sans-serif;font-weight:700;font-size:16px;color:#0E2A47;padding:6px 8px;border-radius:10px;white-space:nowrap')}>
          ...
          <button type="button" onClick={openQuote} className="hv-redd" style={sx('background:#E32027;color:#fff;border:0;border-radius:10px;padding:12px 18px;font-family:Barlow,sans-serif;font-weight:700;font-size:15px;cursor:pointer;white-space:nowrap')}>Free Quote</button>
        </div>
      ...
      {menuOpen ? (
        <div className="lae-mobile" style={sx('position:fixed;inset:72px 0 0 0;background:#fff;z-index:49;overflow-y:auto;padding:12px 20px 120px;animation:lae-fade .2s ease')}>
          {NAV.map((item, i) => (
            <div key={item.label} style={sx('border-bottom:1px solid #EDF1F5')}>
              <div style={sx('display:flex;align-items:center;justify-content:space-between')}>
                <Link href={item.href} onClick={closeMenu} style={sx('display:block;padding:15px 0;font-family:Barlow,sans-serif;font-weight:700;font-size:19px;color:#14263A;flex:1')}>{item.label}</Link>
                {item.groups ? (
              ...
              {mobileOpen[i] && item.groups ? (
                <div style={sx('padding:0 0 14px 4px;display:flex;flex-direction:column;gap:14px')}>
                  {item.groups.map((g) => (
                    <div key={g.label}>
                      {g.href ? (
                        <Link href={g.href} onClick={closeMenu} style={sx('display:block;font-family:Barlow,sans-serif;font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:.06em;color:#1279BF;padding:6px 0')}>{g.label}</Link>
                      ) : (
                        <div style={sx('font-family:Barlow,sans-serif;font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:.06em;color:#5B6E82;padding:6px 0')}>{g.label}</div>
                      )}
                      {g.children.map((ch) => (
          ...
          <div style={sx('display:flex;flex-direction:column;gap:10px;margin-top:20px')}>
            <button type="button" onClick={() => { openQuote(); setMenuOpen(false); }} style={sx('background:#E32027;color:#fff;border:0;border-radius:12px;padding:16px;font-family:Barlow,sans-serif;font-weight:700;font-size:17px;cursor:pointer')}>Get a Free Quote</button>
            <a href="tel:1300010393" style={sx('background:#0E2A47;color:#fff;border-radius:12px;padding:16px;text-align:center;font-family:Barlow,sans-serif;font-weight:700;font-size:17px')}>Call 1300 010 393</a>
            <a href="https://wa.me/61447440050" target="_blank" rel="noopener" style={sx('background:#EAF7DC;color:#3D7A0B;border-radius:12px;padding:16px;text-align:center;font-family:Barlow,sans-serif;font-weight:700;font-size:17px')}>WhatsApp 0447 440 050</a>
          </div>
        </div>
      ) : null}
    </header>
