...
import { FOOTER_AC, FOOTER_EL, FOOTER_CO } from '@/lib/site';
import type { Img } from '@/lib/images';

export default function Footer({ logo, stickySpacer }: { logo: Img; stickySpacer: boolean }) {
  return (
    <footer style={sx('background:#0E2A47;color:#C9D7E6;margin-top:auto')}>
      <div style={sx('max-width:1220px;margin:0 auto;padding:clamp(40px,5vw,64px) 20px 28px;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,220px),1fr));gap:36px')}>
        ...
        <div>
          <div style={sx('font-family:var(--font-heading);font-weight:700;font-size:14px;letter-spacing:.1em;text-transform:uppercase;color:#fff;margin-bottom:14px')}>Air conditioning</div>
        ...
        <div>
          <div style={sx('font-family:var(--font-heading);font-weight:700;font-size:14px;letter-spacing:.1em;text-transform:uppercase;color:#fff;margin-bottom:14px')}>Electrical</div>
          <div style={sx('display:flex;flex-direction:column;gap:9px;font-size:15px')}>
            {FOOTER_EL.map((l) => <Link key={l.slug} href={l.href} className="hv-w" style={sx('color:#C9D7E6')}>{l.label}</Link>)}
          </div>
        </div>
        <div>
          <div style={sx('font-family:var(--font-heading);font-weight:700;font-size:14px;letter-spacing:.1em;text-transform:uppercase;color:#fff;margin-bottom:14px')}>Contact</div>
          <div style={sx('display:flex;flex-direction:column;gap:10px;font-size:15px')}>
            <a href="tel:1300010393" className="hv-lime" style={sx('color:#fff;font-family:var(--font-heading);font-weight:800;font-size:22px;line-height:1.1')}>1300 010 393</a>
